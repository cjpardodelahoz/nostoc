...
#SBATCH -c 1
#SBATCH --error=log/get_16s_from_ffn_set8_%A_%a.err
#SBATCH --output=log/get_16s_from_ffn_set8_%A_%a.out
...
ffn_file=analyses/cyano_genomes_annotation/set8/${bin}/${bin}.ffn
...
if [[ -n ${seq_id} ]]; then
 ...
 # Path to output file
 outfile=analyses/cyano_genomes_annotation/set8/${bin}/16s.fas
