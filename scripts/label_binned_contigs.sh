...
n=1
# Label contigs for each bin
while [ -f ${bin_dir}/${sample_id}_bin.${n}.fa ]
do
 grep -e ">" ${bin_dir}/${sample_id}_bin.${n}.fa | sed 's/>//' | \
 sed 's/$/,'"${n}"'/' > ${out_dir}/${sample_id}_bin.${n}
 ((++n))
done
# Merge labeled contigs from all bins
cat ${out_dir}/${sample_id}_bin.* > ${out_dir}/${sample_id}_contig_labels.csv
# Remove intermediate files
rm ${out_dir}/${sample_id}_bin.*
