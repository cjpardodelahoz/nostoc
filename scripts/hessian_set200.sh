...
#SBATCH --output=log/hessian_set200_%A_%a.out
#SBATCH --error=log/hessian_set200_%A_%a.err
...
# Modify control file to get gradient and Hessian with proper model and parameters
sed -i "s|model\s=\s0|model = 2 * 2: Empirical|" tmp0001.ctl
sed -i "s|aaRatefile\s=\s|aaRatefile = lg.dat|" tmp0001.ctl
# Get gradient, Hessian and branchlenghts with LG model
rm out.BV
rm rst*
codeml tmp0001.ctl
