...
#SBATCH --output=log/ncbi_fcs_set103c_%A_%a.out
#SBATCH --error=log/ncbi_fcs_set103c_%A_%a.err
...
mkdir -p analyses/genome_qc/set103c/fcs/${genome%.fa}_chromosome.fa 
# Run FCS
run_fcsadaptor.sh --fasta-input analyses/cyano_genomes/set103c/${genome%.fa}_chromosome.fa  \
 --output-dir analyses/genome_qc/set103c/fcs/${genome%.fa}_chromosome.fa \
